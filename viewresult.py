...
for x in range(0,len(fiodata['client_stats'])):
	...
	print 'Avg write lat:   %sms'% (fiodata['client_stats'][x]['write']['lat']['mean']/1000000)
	print 'Max write lat:   %sms'% (fiodata['client_stats'][x]['write']['lat']['max']/1000000)
	print 'STDev write:     %sms'% (fiodata['client_stats'][x]['write']['lat']['stddev']/1000000)
	print ''
	print 'Read b/w:        %s MiB/s'% ((fiodata['client_stats'][x]['read']['bw'])/1024)
	print 'Read IOPS:       %s '% (fiodata['client_stats'][x]['read']['bw'])
	print 'Avg read lat:    %sms'% (fiodata['client_stats'][x]['read']['lat']['mean']/1000000)
	print 'Max read lat:    %sms'% (fiodata['client_stats'][x]['read']['lat']['max']/1000000)
	print 'STDev write:     %sms'% (fiodata['client_stats'][x]['read']['lat']['stddev']/1000000)
